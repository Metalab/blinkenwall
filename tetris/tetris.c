...
int main(int argc, char * argv[]) {
	...
	if (err = check_for_touch(playfield, stone, stonex, stoney)){
		...
		for (x=0; x<STONE_SIZE; ++x) {
			...
			int tmpx = GRAVITY_X * -1;
			int tmpy = GRAVITY_Y * -1;
			playfield[x+stonex+tmpx][y+stoney+tmpy][0]=stone[x][y][0];
			playfield[x+stonex+tmpx][y+stoney+tmpy][1]=stone[x][y][1];
			playfield[x+stonex+tmpx][y+stoney+tmpy][2]=stone[x][y][2];			
		    }
		}
	    }
	    stonex=STONE_X_START;
